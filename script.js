document.addEventListener('DOMContentLoaded', () => {
    ...
    const classPeriods = [
        { id: 'p1', name: '1限', start: '08:30', end: '10:10', slots: ["08:30", "08:40", "08:50", "09:00", "09:10", "09:20", "09:30", "09:40", "09:50", "10:00"] },
        { id: 'p2', name: '2限', start: '10:20', end: '12:00', slots: ["10:20", "10:30", "10:40", "10:50", "11:00", "11:10", "11:20", "11:30", "11:40", "11:50"] },
        { id: 'l', name: '昼休み', start: '12:00', end: '12:50', slots: ["12:00", "12:10", "12:20", "12:30", "12:40"] },
        { id: 'p3', name: '3限', start: '12:50', end: '14:30', slots: ["12:50", "13:00", "13:10", "13:20", "13:30", "13:40", "13:50", "14:00", "14:10", "14:20"] },
        { id: 'p4', name: '4限', start: '14:40', end: '16:20', slots: ["14:40", "14:50", "15:00", "15:10", "15:20", "15:30", "15:40", "15:50", "16:00", "16:10"] },
        { id: 'p5', name: '5限', start: '16:30', end: '18:10', slots: ["16:30", "16:40", "16:50", "17:00", "17:10", "17:20", "17:30", "17:40", "17:50", "18:00"] },
        { id: 'p6', name: '6限', start: '18:20', end: '20:00', slots: ["18:20", "18:30", "18:40", "18:50", "19:00", "19:10", "19:20", "19:30", "19:40", "19:50"] }
    ];

    const timeSlots = classPeriods.flatMap(p => p.slots);
    ...
    // GAS Web App URL
    const GAS_API_URL = 'https://script.google.com/macros/s/AKfycbwRGv_X62H4NIMDvW_GLW9Efi7RGFbRm-DRqwrIcoZojmkxoChSaghR5Ma4D9CrC47aJQ/exec';
    ...
    const renderDashboard = () => {
        ...
        const groupReservations = (resList) => {
            ...
            return Object.values(groups).map(g => {
                ...
                if (!g.fullRange) {
                    g._rawSlots.sort();
                    const startSlot = g._rawSlots[0];
                    const endSlot = g._rawSlots[g._rawSlots.length - 1];
                    const start = startSlot.split(' - ')[0];
                    ...
                    g.timeSlot = end ? `${start} - ${end}` : start; // Update display time securely
                } else {
    ...
    // Helper: Calculate End Time (10 min after start)
    const getSlotEndTime = (timeStr) => {
    ...
    const renderScheduleList = (dateStr) => {
        ...
        classPeriods.forEach(period => {
            // 1. Analyze Status for Each Slot in this Period
            const slotStatuses = period.slots.map(slot => {
                const isOpen = daysOpen.some(s => s === slot || s.startsWith(slot + ' '));
                const resInSlot = daysReservations.filter(r => r.timeSlot === slot || r.timeSlot.startsWith(slot + ' '));
                const isBooked = resInSlot.length > 0;
                ...
                if (!isOpen) {
                    ...
                    // Check if Fully Blocked (Private) or Shared
                    const hasPrivate = resInSlot.some(r => r.privacyType !== 'open');
                    if (hasPrivate) {
                        type = 'booked';
                        resData = resInSlot.find(r => r.privacyType !== 'open') || resInSlot[0];
                        label = resData.name;
                    } else {
                        type = 'shared';
                        // Collect names
                        const names = resInSlot.map(r => r.name).join(', ');
                        label = names;
                        resData = resInSlot[0];
                    }
                } else {
                ...
                return { slot, type, label, resData, rawRes: resInSlot };
            });

            // 2. Group Consecutive Statuses
            const groups = [];
            let currentGroup = null;

            slotStatuses.forEach(s => {
                // Check equality with previous
                if (currentGroup && s.type === currentGroup.type && s.label === currentGroup.label) {
                    currentGroup.slots.push(s.slot);
                    currentGroup.endSlot = s.slot;
                } else {
            ...
            summary.textContent = `${period.name} (${period.start} ~ ${period.end})`;
            details.appendChild(summary);

            const contentDiv = document.createElement('div');
            contentDiv.style.padding = '8px';

            if (groups.length > 0) hasAnySlots = true;

            // 3. Render Groups
            groups.forEach(g => {
                const groupDiv = document.createElement('div');
                groupDiv.className = `schedule-item ${g.type}`;

                // Time Range String
                const endTimeStr = getSlotEndTime(g.endSlot);
                const timeRangeStr = `${g.startSlot} - ${endTimeStr}`;

                // Content based on Type
                if (g.type === 'blocked') {
                    groupDiv.innerHTML = `
                        <div style="font-weight:600; color:#a0aec0; width:120px;">${timeRangeStr}</div>
                        <div class="status" style="display:flex; align-items:center; gap:8px;">
                            <span class="badge" style="background:#edf2f7; color:#a0aec0; border:1px solid #cbd5e0;">✕</span>
                            <span style="font-size:0.9rem; color:#a0aec0;">受付停止</span>
                        </div>
                     `;
                } else if (g.type === 'available') {
                    groupDiv.innerHTML = `
                        <div class="time" style="width:120px;">${timeRangeStr}</div>
                        <div style="flex:1; display:flex; justify-content:space-between; align-items:center;">
                            <span style="font-size:0.9rem; color:#2f855a; font-weight:600;"><i class="far fa-circle"></i> 空きあり</span>
                            <button class="select-slot-btn" style="background:var(--color-primary);">予約する</button>
                        </div>
                     `;
                    groupDiv.addEventListener('click', () => {
                        moveToBookingForm(g.slots);
                        // Set privacy default
                        const privacyRadios = document.getElementsByName('privacyType');
                        privacyRadios.forEach(r => { if (r.value === 'private') r.checked = true; });
                    });
                } else if (g.type === 'booked') {
                    // Booked (Private)
                    let actionHtml = '';
                    // Check Permission
                    let canEdit = false;
                    if (currentUserAccount && g.resData) {
                        const r = g.resData;
                        const isOwner = r.studentId === currentUserAccount.studentId;
                        const isAttendee = r.additionalAttendees && Array.isArray(r.additionalAttendees) && r.additionalAttendees.some(a => a.id === currentUserAccount.studentId);
                        const isAdmin = currentUserAccount.status === 'faculty' || currentUserAccount.status === 'admin';
                        if (isOwner || isAttendee || isAdmin) canEdit = true;
                    }
                    ...
                    groupDiv.innerHTML = `
                        ...
                        <div class="status" style="flex:1; display:flex; justify-content:space-between; align-items:center;">
                            <div style="display:flex; align-items:center; gap:8px;">
                                ...
                                <span class="booker-name">${g.label} 様</span>
                            </div>
                            ${canEdit ? `<button class="slot-edit-btn" style="font-size:0.8rem; padding:4px 8px; background:#3182ce; color:white; border:none; border-radius:4px; cursor:pointer;">詳細</button>` : ''}
                        </div>
                     `;
                    if (canEdit) {
                        groupDiv.style.cursor = 'pointer';
                        groupDiv.addEventListener('click', () => showReservationDetails(g.resData, dateStr));
                    }
                } else if (g.type === 'shared') {
                    // Shared
                    groupDiv.style.borderLeft = '4px solid #ecc94b';
                    const names = g.resData ? g.label : '';

                    groupDiv.innerHTML = `
                        <div class="time" style="width:120px;">${timeRangeStr}</div>
                         <div style="flex:1; display:flex; flex-direction:column; align-items:flex-end; gap:4px;">
                            <div style="width:100%; display:flex; justify-content:space-between; align-items:center;">
                                 <span style="font-size:0.9rem; color:#d97706; font-weight:600;"><i class="fas fa-users"></i> 同席可</span>
                                 <button class="select-slot-btn" style="background:#d97706; font-size:0.8rem;">予約(同席)</button>
                            </div>
                    ...
                    groupDiv.addEventListener('click', () => {
                        ...
                        // Set privacy default
                        const privacyRadios = document.getElementsByName('privacyType');
                        privacyRadios.forEach(r => { if (r.value === 'open') r.checked = true; });
                    });
                }
            ...
            dailyScheduleList.appendChild(details);
        });

        if (!hasAnySlots) {
            dailyScheduleList.innerHTML = '<div style="padding:20px; text-align:center; color:#718096;">予約可能な枠がありません</div>';
        }
    };

    const moveToBookingForm = (slots) => {
        // slots is array of valid start times for this block
        if (!Array.isArray(slots)) slots = [slots];

        // 1. Setup Time Selector in Form
        const displayContainer = document.getElementById('selectedTimeDisplay').parentElement;
        // Check if we already replaced it or need to
        let timeSelect = document.getElementById('startTimeSelect');
        const timeInput = document.getElementById('selectedTimeDisplay');

        if (!timeSelect) {
            // Create Select
            timeSelect = document.createElement('select');
            timeSelect.id = 'startTimeSelect';
            timeSelect.style.cssText = 'width:100%; padding:0.6rem; border:1px solid #e2e8f0; border-radius:6px; background:#f7fafc; font-weight:bold; color:var(--color-primary);';
            timeSelect.addEventListener('change', (e) => {
                document.getElementById('timeSlot').value = e.target.value;
            });
            // Hide Input, Show Select
            timeInput.style.display = 'none';
            displayContainer.insertBefore(timeSelect, timeInput);
        } else {
            timeInput.style.display = 'none';
        ...
        document.getElementById('timeSlot').value = slots[0];
    ...
    if (batchRegistrationBtn) {
        batchRegistrationBtn.addEventListener('click', () => {
            ...
            timeSlots.forEach((slot, index) => {
                const start = slot;
                // Calculate end time dynamically for 10-min slots
    ...
    reservationForm.addEventListener('submit', (e) => {
        ...
        const durationSlots = parseInt(document.getElementById('duration').value); // 1=15m, 2=30m, 3=45m, 4=60m...
        const startTimeSlot = document.getElementById('timeSlot').value; // e.g., "10:00 - 10:15" (Wait, timeSlots are just Start Times like "10:00")
...
`;
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', (e) => {
            ...
            // New Asynchronous Server-Side Registration
            const submitBtn = registerForm.querySelector('button[type="submit"]') || registerForm.querySelector('.submit-btn') || registerForm.querySelector('button');
            ...
            const url = `${GAS_API_URL}${GAS_API_URL.includes('?') ? '&' : '?'}cb=${Date.now()}`;

            fetch(url, {
                method: 'POST',
                redirect: 'follow', // Make sure fetch follows redirects transparently (GAS req)
                headers: { 'Content-Type': 'text/plain' },
                body: JSON.stringify(payload)
            })
                .then(res => res.json())
                .then(result => {
                    ...
                    if (result.success) {
                        // Registration successful on server, override local with the latest list from server
                        if (result.updatedUsers) {
                            localStorage.setItem('miyamoto_users', JSON.stringify(result.updatedUsers));
                        }

                        currentUserAccount = newUser;
                        localStorage.setItem('miyamoto_current_user', JSON.stringify(newUser));

                        if (typeof accountModal !== 'undefined' && accountModal) {
    ...
    if (deleteAccountBtn) {
        deleteAccountBtn.addEventListener('click', () => {
            if (confirm('本当にアカウントを削除しますか？\nこの操作は取り消せません。')) {
                if (currentUserAccount) {
                    const sidToDelete = currentUserAccount.studentId;

                    // Sync to GAS
                    const payload = { action: 'deleteUser', studentId: sidToDelete };
                    fetch(GAS_API_URL, {
                        method: 'POST',
                        redirect: 'follow',
                        headers: { 'Content-Type': 'text/plain' },
                    ...
                    // Local logout process
                    const currentUsers = getUsers();
                    const newUsers = currentUsers.filter(u => u.studentId !== sidToDelete);
                    localStorage.setItem('miyamoto_users', JSON.stringify(newUsers));
        ...
        usersWithCount.forEach(user => {
            ...
            deleteBtn.onclick = () => {
                if (confirm(`${user.name} (${user.studentId}) を削除してもよろしいですか？`)) {
                    // Sync to GAS
                    const payload = { action: 'deleteUser', studentId: user.studentId };
                    fetch(GAS_API_URL, {
                        method: 'POST',
                        redirect: 'follow',
                        headers: { 'Content-Type': 'text/plain' },
                    ...
                    localStorage.setItem('miyamoto_users', JSON.stringify(updatedUsers));

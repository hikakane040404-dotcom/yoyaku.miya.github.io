document.addEventListener('DOMContentLoaded', () => {
    ...
    const GAS_API_URL = 'https://script.google.com/macros/s/AKfycbyHHrf3EEhXUIUcV_gjlwDA6DqFNRGrguPqMaLaiStzGAALDVRMQaIlP5ATRrSCDAdPYw/exec';
    ...
    reservationForm.addEventListener('submit', (e) => {
        ...
        try {
            // Pass the ALREADY FILTERED reservations object to avoid duplicates
            saveReservation(datesStr, formData, reservations);
...
`;
    ...
    const openStudentHistoryModal = () => {
        ...
        // Populate Select
        const reservations = getReservations() || {};
        const studentMap = new Map(); // id -> {name, count}
            ...
            dayList.forEach(r => {
                if (r.studentId && r.name) {
                    if (!studentMap.has(r.studentId)) {
                        studentMap.set(r.studentId, { name: r.name, lastDate: r.timestamp || '' });
                    }
                }
            });
        });

        // 2. Scan Users (Registered but maybe no reservations)
        const users = getUsers() || [];
        users.forEach(u => {
            if (u.studentId && !studentMap.has(u.studentId)) {
                studentMap.set(u.studentId, { name: u.name, lastDate: '' });
            }
        });

        // Build Options
        select.innerHTML = '<option value="">-- 学生を選択 --</option>';

        // Sort by ID
        const sortedIds = Array.from(studentMap.keys()).sort();
        ...
        if (content) content.innerHTML = '<div style="text-align:center; color:#718096; padding: 20px;">学生を選択すると履歴が表示されます</div>';

        modal.classList.add('show');

        // Close Admin Menu if open
        const adminMenu = document.getElementById('adminMenuModal');
        ...
        const createCard = (r, isFuture = false) => {
            ...
            return `
                <div style="background:${bg}; border:1px solid #e2e8f0; border-radius:8px; padding:1rem; margin-bottom:1rem; box-shadow:0 1px 2px rgba(0,0,0,0.05); ${styleBorder} position:relative;">
                    ...
                    <div style="display:flex; justify-content:space-between; margin-bottom:0.5rem; border-bottom:1px solid #edf2f7; padding-bottom:0.5rem; padding-right: 130px;">
                        <div style="font-weight:bold; color:#2b6cb0;">
                            ${icon} <i class="far fa-calendar-alt"></i> ${dateDisplay} ${r.timeSlot}
                        </div>
                        <div style="font-size:0.85rem; color:#718096;">
                            [${translateFormat(r.format)}] ${translateType(r.type)}
                        </div>
                    </div>
                    ...
                    <div style="display:grid; gap:10px; margin-top:10px;">
                        ${r.noteStudent ? `
                        <div style="background:#f7fafc; padding:8px; border-radius:4px; border-left:3px solid #cbd5e0;">
                            <div style="font-size:0.8rem; font-weight:bold; color:#718096; margin-bottom:2px;">学生メモ</div>
                            <div style="font-size:0.9rem; white-space:pre-wrap;">${r.noteStudent}</div>
                        </div>` : ''}
                        
                        ${r.noteShared ? `
                        <div style="background:#fffaf0; padding:8px; border-radius:4px; border-left:3px solid #ed8936;">
                            <div style="font-size:0.8rem; font-weight:bold; color:#dd6b20; margin-bottom:2px;">共有議事録</div>
                            <div style="font-size:0.9rem; white-space:pre-wrap;">${r.noteShared}</div>
                        </div>` : ''}
                        
                        ${r.noteFaculty ? `
                        <div style="background:#fff5f5; padding:8px; border-radius:4px; border-left:3px solid #fc8181;">
                            <div style="font-size:0.8rem; font-weight:bold; color:#e53e3e; margin-bottom:2px;">教員メモ (非公開)</div>
                            <div style="font-size:0.9rem; white-space:pre-wrap;">${r.noteFaculty}</div>
                        </div>` : ''}
                    </div>
                </div>
            `;
                    ...
                    try {
                        ...
                        const checkClose = setInterval(() => {
                            ...
                            if (modal && !modal.classList.contains('show')) {
                                ...
                                // Refresh logic
                                const currentSelect = document.getElementById('historyStudentSelect');
                                if (currentSelect && currentSelect.value) {
                                    setTimeout(() => renderStudentHistory(currentSelect.value), 100);
                                }
                            }
                        }, 500);
            ...
            return `
                <div style="background:${bg}; border:1px solid #e2e8f0; border-radius:8px; padding:1rem; margin-bottom:1rem; box-shadow:0 1px 2px rgba(0,0,0,0.05); ${styleBorder} position:relative;">
                     <button class="open-detail-student-trigger" data-res="${safeData}" data-date="${r.date}"
                        style="position:absolute; top:10px; right:10px; background:white; border:1px solid #cbd5e0; color:#4a5568; border-radius:4px; padding:4px 10px; cursor:pointer; font-size:0.8rem;">
                        <i class="fas fa-search-plus"></i> 詳細・メモ編集
                    </button>
                    <div style="display:flex; justify-content:space-between; margin-bottom:0.5rem; border-bottom:1px solid #edf2f7; padding-bottom:0.5rem; padding-right: 155px;">
                        <div style="font-weight:bold; color:#2b6cb0;">
                            <i class="far fa-calendar-alt"></i> ${dateDisplay} ${r.timeSlot}
                        </div>
                        <div style="font-size:0.85rem; color:#718096;">
                            [${translateFormat(r.format)}] ${translateType(r.type)}
                        </div>
                    ...
                    <div style="display:grid; gap:8px;">
                        ${r.noteStudent ? `
                        <div style="background:#f7fafc; padding:8px; border-radius:4px; border-left:3px solid #cbd5e0;">
                            <div style="font-size:0.8rem; font-weight:bold; color:#718096; margin-bottom:2px;">マイメモ (自分用)</div>
                            <div style="font-size:0.85rem; white-space:pre-wrap;">${r.noteStudent}</div>
                        </div>` : ''}
                        
                        ${r.noteShared ? `
                        <div style="background:#fffaf0; padding:8px; border-radius:4px; border-left:3px solid #ed8936;">
                            <div style="font-size:0.8rem; font-weight:bold; color:#dd6b20; margin-bottom:2px;">共有議事録・フィードバック</div>
                            <div style="font-size:0.85rem; white-space:pre-wrap;">${r.noteShared}</div>
                        </div>` : ''}
                    </div>
                </div>
            `;
    ...
    if (historyStudentSelectRef) {
        historyStudentSelectRef.addEventListener('change', (e) => {
            renderStudentHistory(e.target.value);
        });
    }

document.addEventListener('DOMContentLoaded', () => {
    ...
    const saveDateConfig = (c) => localStorage.setItem('miyamoto_date_config', JSON.stringify(c));
    ...
    const openAdminModal = (dateObj, dateStr) => {
        ...
        const isSystemAdmin = !currentAdminUser && getOwners().length === 0; // Allow if no owners exist
    ...
    // GAS API Endpoint for Email
    const GAS_API_URL = 'https://script.google.com/macros/s/AKfycbye3DDNLj48BGxIctjryVDAgiT-55E8Lz5wGK3bRxcL5cJUbuO8xMACIw-D2cEkWjxo/exec';
...
`;
    ...
    const updateAdminUI = () => {
        if (isAdminMode) {
            ...
            const isFaculty = currentAdminUser && currentAdminUser.status === 'faculty';
            const isAdmin = currentAdminUser && currentAdminUser.status === 'admin';
            const isSystemAdmin = !currentAdminUser && getOwners().length === 0;

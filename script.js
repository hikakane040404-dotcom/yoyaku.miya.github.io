document.addEventListener('DOMContentLoaded', () => {
    ...
    const GAS_API_URL = 'https://script.google.com/macros/s/AKfycbxJyubJ7Sij4DU8Ls84I0aek3gf2AGo4o4G6BYIxolXYBFrtd-B0vJD2eSjlCZquFoBNg/exec';
                ...
                const processAndStore = (localKey, remoteKey) => {
                    ...
                    if (remoteKey === 'users' || remoteKey === 'owners') {
                        ...
                        if (Array.isArray(parsed) && Array.isArray(localList)) {
                            // Map local by ID (studentId for users, name/email for owners)
                            const localMap = {};
                            localList.forEach(item => {
                                const key = (remoteKey === 'users') ? item.studentId : (item.email || item.name);
                                if (key) localMap[key] = item;
                            });

                            // Merge: Remote is master, but inherit password/notify from local if remote is empty
                            parsed = parsed.map(remoteItem => {
                                const key = (remoteKey === 'users') ? remoteItem.studentId : (remoteItem.email || remoteItem.name);
                                const localItem = localMap[key];
                                if (localItem) {
                                    // If remote password is missing/empty but local has it, use local
                                    if (!remoteItem.password && localItem.password) {
                                        remoteItem.password = localItem.password;
                                    }
                                    // If remote notify is somehow undefined/defaulting, stick to remote unless distinct reason not to.
                                    // Actually, remote 'notify' from sheet is 'ON'/'OFF' mapped to boolean in GAS.
                                    // So remote should be correct. But just in case of initial migration:
                                    if (remoteItem.notify === undefined && localItem.notify !== undefined) {
                                        remoteItem.notify = localItem.notify;
                                    }
                                }
                                return remoteItem;
                            });
        ...
        if (canEdit) {
            ...
            sameDayLabel.innerHTML = `
                        < input type = "checkbox" id = "adminSameDayToggle" >
                            <div>当日予約を許可<span style="font-weight:normal; font-size:0.85rem; display:block;">(この日のみ)</span></div>
                    `;

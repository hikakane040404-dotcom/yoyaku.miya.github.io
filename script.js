document.addEventListener('DOMContentLoaded', () => {
    ...
    const GAS_API_URL = 'https://script.google.com/macros/s/AKfycbxoZdeLzuDulgmgk3LYvhjfPpzcyMKXumgsJsldvMmX8vNUhClYQC14YRWexgfHG_Ve/exec';
    ...
    const syncDataWithGas = async (isSilent = false) => {
        ...
        try {
            ...
            if (remoteData && typeof remoteData === 'object') {
                console.log('Sync Success. Received keys:', Object.keys(remoteData));

                const processAndStore = (localKey, remoteKey) => {
                    let val = remoteData[remoteKey];
                    if (val === undefined || val === null) return;

                    // Crucial: Handle potential double-quotes and types
                    let stringToStore = typeof val === 'string' ? val : JSON.stringify(val);

                    // If it's a double-quoted string (e.g. ""{...}""), fix it
                    if (stringToStore.startsWith('"{') && stringToStore.endsWith('}"')) {
                        try {
                            const inner = JSON.parse(stringToStore);
                            stringToStore = typeof inner === 'string' ? inner : JSON.stringify(inner);
                        } catch (e) { }
                    }
    ...
    const renderDashboard = () => {
        ...
        if (todayGroups.length === 0) {
            ...
            todayGroups.forEach(r => {
                ...
                // Check if deletable by current user
                let deleteAction = '';
                if (currentUserAccount || isAdminMode) {
                    ...
                    if (isOwner || isAttendee || isAdmin) {
                        deleteAction = `<button class="cancel-my-res-btn" data-date="${todayStr}" data-timestamp="${r.timestamp}" style="margin-top:4px; padding:2px 6px; font-size:0.7rem; color:white; background:#e53e3e; border:none; border-radius:4px; cursor:pointer;">キャンセル</button>`;
                    }
                }
                ...
                if (deleteAction) {
                    const btn = item.querySelector('.cancel-my-res-btn');
                    btn.addEventListener('click', (e) => {
                        e.stopPropagation(); // Prevent bubbling if any
                        if (confirm('この予約を取り消しますか？\n(予約情報のすべての枠が削除されます)')) {
                            cancelReservation(btn.dataset.date, btn.dataset.timestamp);
                        }
                    });
                }
    ...
    const showReservationDetails = (r, dateStr) => {
        ...
        if (r.attendees > 1 && r.additionalAttendees && r.additionalAttendees.length > 0) {
            attendeesHtml += '<div style="margin-top:0.5rem; padding-left:1rem; border-left:2px solid #cbd5e0;"><strong>同席者:</strong><br>';
            r.additionalAttendees.forEach(a => {
                attendeesHtml += `${a.name} (${a.id})<br>`;
            });
            attendeesHtml += '</div>';
        }
        ...
        if (currentUserAccount) {
            ...
            if (isOwner || isAdmin) canEdit = true;
        }

        if (canEdit) {
            const editBtnDiv = document.createElement('div');
            editBtnDiv.style.marginTop = '1.5rem';
            editBtnDiv.style.textAlign = 'center';
            editBtnDiv.innerHTML = `<button id="editReservationBtn" class="submit-btn" style="background:#3182ce;">予約内容を修正する</button>`;
            reservationDetailsBody.appendChild(editBtnDiv);
...
`;
    ...
    const renderUserList = () => {
        ...
        users.forEach(user => {
            ...
            li.style.gridTemplateColumns = '1fr 1fr 1.5fr 50px';

document.addEventListener('DOMContentLoaded', () => {
    ...
    const pushDataToGas = async (key, data) => {
        try {
            // Ensure data is sent as a single JSON string for the cell
    ...
    const syncDataWithGas = async () => {
        ...
        try {
            ...
            const urlWithCacheBuster = GAS_API_URL + (GAS_API_URL.includes('?') ? '&' : '?') + 't=' + Date.now();

            const response = await fetch(urlWithCacheBuster, {
                signal: controller.signal,
                cache: 'no-store' // Also hint browser not to cache
            });
            clearTimeout(id);
            ...
            if (remoteData && typeof remoteData === 'object') {
                ...
                // If Owner Management modal is open, refresh its list too

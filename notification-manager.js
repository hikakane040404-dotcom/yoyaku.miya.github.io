...
class NotificationManager {
    ...
    async saveTokenToServer(token) {
        try {
            ...
            const GAS_API_URL = localStorage.getItem('miyamoto_gas_url') ||
                'https://script.google.com/macros/s/AKfycbwRGv_X62H4NIMDvW_GLW9Efi7RGFbRm-DRqwrIcoZojmkxoChSaghR5Ma4D9CrC47aJQ/exec';
    ...
    async unsubscribe() {
        try {
            ...
            const token = this.currentToken || localStorage.getItem('miyamoto_fcm_token');

            if (token) {
                // Delete token from server
                await this.deleteTokenFromServer(token);
